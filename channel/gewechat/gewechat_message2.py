...
class GeWeChatMessage(ChatMessage):
    def __init__(self, msg, client: GewechatClient):
        ...
        notes_join_group = ["你邀请","加入群聊", "加入了群聊", "invited", "joined"]  # 可通过添加对应语言的加入群聊通知中的关键词适配更多
        notes_bot_join_group = ["你邀请","邀请你", "invited you", "You've joined", "你通过扫描"]
        ...
        if msg_type == 1:  # Text message
            ...
            if self.is_group:
                ...
                #     logger.debug("机器人加入群聊消息，不处理~")
                #     pass
            if any(note_join_group in content for note_join_group in notes_join_group):  # 若有任何在notes_join_group列表中的字符串出现在NOTE中
                try:
        ...
        if self.is_group:
            ...
            if isinstance(self.content, str):
                ...
                if isinstance(self.content, dict):
                    self.content = re.sub(f'{self.actual_user_id}:\n', '', self.content.get('string',''))  # 去掉wxid前缀
                    self.content = re.sub(r'@[^\u2005]+\u2005', '', self.content.get('string',''))  # 去掉@信息
                else:
                     self.content = ''
