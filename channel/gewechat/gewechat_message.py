...
class GeWeChatMessage(ChatMessage):
    def __init__(self, msg, client: GewechatClient):
        ...
        #增加机器人自己发送消息是判断为group消息，如果机器人在群里主动发消息，ToUserName为xxxxxx@chatroom
        if not self.is_group:
            if "@chatroom" in msg['Data']['ToUserName']['string']:
                self.is_group = True
        self.my_msg = self.msg['Wxid'] == self.from_user_id  # 消息是否来自自己
        if self.my_msg and self.is_group:
            self.from_user_id = msg['Data']['ToUserName']['string']
            self.to_user_id = msg['Data']['FromUserName']['string']
            self.other_user_id = self.from_user_id      
