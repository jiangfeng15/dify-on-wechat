...
class GeWeChatChannel(ChatChannel):
    ...
    def startup(self):
        ...
        def set_scheduler():
            #启动定时器任务
            scheduler_timer = scheduler(self)
            scheduler_timer.init_timer(conf().get('task_config'))
            scheduler_timer.timer_start()
            logger.info("[gewechat] 启动定时任务 successfully")

        callback_thread = threading.Thread(target=set_scheduler, daemon=True)
        callback_thread.start()
...
class Query:
    ...
    def POST(self):
        ...
        # if gewechat_msg.is_group:
        #     logger.debug(f"[gewechat] receive data: 发送消息: {gewechat_msg.content}")
        # 微信客户端的状态同步消息
        ...
        if int(gewechat_msg.create_time) < int(time.time()) - 60 * 5: # 跳过5分钟前的历史消息
            ...
            return "success"
        #if "@chatroom" not in gewechat_msg.to_user_id:
        #    gewechat_msg.is_group = False
        #else:
        #    gewechat_msg.is_group = True
